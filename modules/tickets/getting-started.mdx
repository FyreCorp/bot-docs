...
To update a ticket category, refer to the [options guide](./options) for the various update commands and their options.
...
Here's a list of the command's options and what they're for.

- `enabled`: If the confirmation prompt is enabled in a category.
- `delay`: The delay (wait time) in seconds before a user can confirm they want to open a ticket (default: 0).
- `mode`: The confirmation prompt display mode, plain or embed.
