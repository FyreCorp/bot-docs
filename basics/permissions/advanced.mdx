...
description: "Learn about Fyre's advanced permission system for users and how your server members can use Fyre without needing actual Discord permissions."
---
...
To add or remove permissions from a role, use the `/permissions` (`/permissions @role [permission-string]`) command.
...
You can also use the command without a permission string to view the role's current permissions.
...
<Tabs>
    ...
    <Tab title="Configuration">
        - `config`: Use `/config view` and view a server's configuration.
        ...
        - `config.member-backup`: Use `/config member-backup` and change a server's member backup settings.
    </Tab>
